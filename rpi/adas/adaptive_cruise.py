...
class AdaptiveCruiseControl:
    ...
    def update(self, sensor_data):
        ...
        if distance <= self.e_stop_dist:
            ...
            # Speed ramps from 0 at e_stop_dist to cruise*0.70 at min_follow_dist
            # (matches FOLLOW zone entry speed for a smooth transition)
            result.mode = "DECEL"
            ratio = (distance - self.e_stop_dist) / (
                self.min_follow_dist - self.e_stop_dist)   # 0→1
            result.speed_norm = cruise * ratio * 0.70      # matches FOLLOW speed
