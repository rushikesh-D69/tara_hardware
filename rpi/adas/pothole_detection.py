...
class PotholeDetector:
    ...
    def _detect_classifier(self, road_roi, result, frame_width):
        ...
        if pothole_prob >= self.conf_threshold:
            ...
            # Estimate pothole position using edge density analysis
    ...
    def _estimate_position(self, road_roi):
        """
        Estimate pothole position using edge density analysis.
        Potholes create more edges (texture, depth discontinuities) than
        smooth road surface. Works on any floor color.

        On a black chart-paper floor, the previous Otsu thresholding approach
        failed because the entire floor was "dark". Edge density is invariant
        ...
        # Apply Canny edge detection — edges indicate texture/depth changes
        ...
        # Require minimum edge difference to avoid noise-based decisions
        total_edges = left_edges + center_edges + right_edges
        if total_edges < 100:
            return "center"  # too few edges to determine position
